...
/*   Updated: 2022/11/07 18:38:52 by rliu             ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
Form::Form(std::string const name, unsigned int const gradeSign, unsigned int const gradeExecut):
...
void Form::signForm(Bureaucrat &bc){
    if (this->getSigned())
        std::cout << bc.getName() << " signed" << this->_name;
    else
        std::cout << bc.getName() << " couldn’t sign " << this->_name<<" because " 
        <<std::endl;
}
